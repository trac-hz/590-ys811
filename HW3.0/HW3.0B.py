...
def NN_eval(x,submatrices):
    ly1 = x
    ly2=np.matmul(ly1,submatrices[0].transpose())+submatrices[1].transpose()
    a2 = S(ly2)
    ly3=np.matmul(a2,submatrices[2].transpose())+submatrices[3].transpose()
    a3 = S(ly3)
    ly4=np.matmul(a3,submatrices[4].transpose())+submatrices[5].transpose()
    out = S(ly4)
    return out
...
def plot_1(xcol=1,xla='x',yla='y'):
	...
	ax.plot(X[train_idx][:,xcol].transpose()    , YPRED_T,'.', label='Model') 
